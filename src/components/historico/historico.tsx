...
import { Calendar, Search, Eye, History, TestTube, FileSpreadsheet, Send, FileDown, FileText, Check, Download, ChevronLeft, ChevronRight} from "lucide-react";
...
export function Historico({ userRole }: HistoricoProps) {
  ...
  const enviarEmail = async (idAnalise: number) => {
    if(!confirm("Deseja enviar o boletim desta análise para o e-mail do fornecedor?")) return;

    setSendingEmailId(idAnalise);
    try {
      await api.post(`/relatorios/analise/${idAnalise}/enviar`);
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          {userRole === "OPERADOR" && (
                            <Button 
                              ...
                              onClick={() => enviarEmail(analise.idAnalise)}
                                ...
                                <div className="space-y-4">
                                  ...
                                  <div>
                                    ...
                                    <div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm">
                                      <div className="flex justify-between"><span className="text-muted-foreground">Pureza:</span> <span>{analiseDetalhada.pureza.toFixed(2)}%</span></div>
                                      <div className="flex justify-between"><span className="text-muted-foreground">Pol (Cana):</span> <span>{analiseDetalhada.polCana.toFixed(2)}%</span></div>
                                      <div className="flex justify-between"><span className="text-muted-foreground">Fibra:</span> <span>{analiseDetalhada.fibra.toFixed(2)}%</span></div>
                                      <div className="flex justify-between"><span className="text-muted-foreground">AR (Cana):</span> <span>{analiseDetalhada.arCana.toFixed(2)}%</span></div>
                                      <div className="flex justify-between"><span className="text-muted-foreground">AR (Caldo):</span> <span>{analiseDetalhada.arCaldo.toFixed(2)}%</span></div>
                                      <div className="flex justify-between"><span className="text-muted-foreground">Pol (Caldo):</span> <span>{analiseDetalhada.polCaldo.toFixed(2)}%</span></div>
                                    </div>
